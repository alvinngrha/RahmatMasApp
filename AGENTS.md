...
- Language: Kotlin with Jetpack Compose (Material 3). Indent: 4 spaces; follow Kotlin official style.
- Architecture: MVVM. Keep UI state immutable; one `ViewModel` per screen.
- Naming: screens `...Screen`; state `...ViewModel`/`...ViewModelFactory`; data `...Repository`; Room `...Dao`/`...Entity`.
- Packages mirror features (e.g., `ui/admin/...`, `ui/customer/...`, `data/...`). Prefer clear, descriptive names.

## Testing Guidelines
- Frameworks: JUnit (unit), AndroidX + Compose testing (instrumentation/UI).
- Test classes mirror targets (e.g., `HomeCustomerViewModelTest`). Focus on `ViewModel` logic, repositories, and Room queries.
- Run tests: `./gradlew testDebugUnitTest` and `./gradlew connectedAndroidTest`.

## Commit & Pull Request Guidelines
- Use Conventional Commits (e.g., `feat:`, `fix:`, `refactor:`, `chore:`).
- PRs must include: clear summary, linked issue, test steps; add screenshots/GIFs for UI changes. Keep scope narrow; ensure build, tests, and lint are green.
...
- Review diffs for secrets; prefer environment‑specific or remote config for releases.
